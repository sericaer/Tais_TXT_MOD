...
namespace UnitTest.RunData
{
    ...
    public class TestAllLink
    {
        ...
        public void TestInit()
        {
            ...
            {
                Assert.True(person.relation.buffers.Any(x=>x.key == "PARTY_RELATION"));

                var partyDef = GMRoot.define.parties.Single(x => x.key == person.family.partyName);
                Assert.AreEqual(partyDef.relation.Single(x => x.peer == GMRoot.runner.taishou.partyName).value, person.relation.buffers.Single(x=>x.key == "PARTY_RELATION").value);
            }
        }
