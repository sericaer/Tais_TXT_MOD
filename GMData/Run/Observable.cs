...
namespace GMData.Run
{
    ...
    public class ObsBufferedValue : INotifyPropertyChanged
    {
        ...
        [JsonProperty]
        public IEnumerable<BufferInfo> buffers
        {
            get
            {
            ...
            set
            {
                sourceCache.Edit(list =>
                {
                    list.Clear();
                    list.AddOrUpdate(value);
                });
            }
        }

        public SourceCache<BufferInfo, string> sourceCache;
        ...
        {
            sourceCache = new SourceCache<BufferInfo, string>(x=>x.key);
            if(CalcValue != null)
            {
        ...
        {
            sourceCache.Connect().Subscribe(x => value  = CalcValue(baseValue, sourceCache.Items.Select(b=>b.value)));

            this.OBSProperty(x => x.baseValue).Subscribe(_ => value = CalcValue(baseValue, sourceCache.Items.Select(b => b.value)));
        }
    }
}
